...
class Welcome2 extends CI_Controller {
	...
	function getCockpitList(){
		...
		return array(
			...
			"PSGlobal",
			//Removed:"USMDO"
			"consumermarketknowledge",
			"communications",
			"brandmanagement",
			"FBNLSMO",
			"PGChemicals",
			///////July 4 Commercialization
			"hrnet",
			"imeahq"
		);
	...
	function showHomePage(){
		...
		if($emailContent != ''){
			...
			$emailSubject = "[WARNING] PGCOCKPITS Site Quota Alert Job 2- ";
		...
		//RUN OK Email
			...
			$this->email->cc('jayr.arenas@hp.com, ronna-may.dimapilis@hp.com'); 
			//$this->email->bcc('them@their-example.com'); 
			$this->email->subject("[FYI] PGCOCKPITS Site Quota Run Check - Job 2");
			date_default_timezone_set('Asia/Manila');
