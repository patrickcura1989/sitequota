...
<h3 class="myH3Bigger"><?php echo $ticketDetails->incident_number.' - '.$ticketDetails->title;?></h3>
<hr class="carved"/>
...
<td width = '350'>
	...
	<tr>
		<td><b/>Impact</td>
		<td><?php echo $ticketDetails->impact;?></td>
	</tr>
	<tr>
		<td><b/>Urgency</td>
		<td><?php echo $ticketDetails->urgency;?></td>
	</tr>
	<tr>
		<td><b/>Priority</td>
	...
	<tr>
		<td><b/>CI</td>
		<td><?php echo $ticketDetails->affected_ci;?></td>
	</tr>
	<tr>
		<td><b/>Service</td>
	...
	<tr>
		<td><b/>Queue</td>
		<td><?php echo $ticketDetails->queue;?></td>
	</tr>
	<tr>
		<td><b/>Assignee</td>
...
<td valign=top width='350'>
	<table class="myTable">
		<tr>
			<td><b/>Status</td>
			<?php 
				if($ticketDetails->current_status == 'Closed'){
					echo '<td><div style="background-color:green;"><font color="white"/>'.$ticketDetails->current_status.'</div></td>';
				}
				else
					echo '<td >'.$ticketDetails->current_status.'</td>';
			?>
		...
		<tr>
			<td><b/>SLA (SGT)</td>
			<td><?php echo $ticketDetails->target_date;?></td>
		</tr>
		<tr>
			<td><b/>SLA %</td>
	...
	?>
		<font class="myFont">For Incident Managers Only</font><br/><br/>
		...
		<input type=hidden name="ticketNumber" value="<?php echo $ticketDetails->incident_number;?>"/>
		...
		<?php 
			if($this->sitequota_model->isImManager()){
				...
				if($myTeammates != null){
					...
					echo '</tr></table><br/>';
					echo form_open($form);
					echo '<input type="hidden" name="ticketNumber" value="'.$ticketDetails->incident_number.'"/>';
...
<td width = '300' valign=top>
	...
	<?php
		$queueBounces = $this->sitequota_model->getImTicketDistinctBounces($ticketId);
		if($queueBounces != null)
		foreach($queueBounces as $row){
			echo '<tr>';
			echo '<td>'.$row->owning_team.'</td>';
			$computeTime = $this->sitequota_model->computeImQueueTime($ticketId, $row->owning_team);
...
<td valign=top width = '400'>
	...
	<th>Logged Time</th>
	<?php
		$queueBounces = $this->sitequota_model->getImTicketBounces($ticketId);
		if($queueBounces != null)
		foreach($queueBounces as $row){
			echo '<tr>';
			echo '<td>'.$row->owning_team.'</td>';
...
<td valign=top>
	...
	<?php
		...
		$updateList = $this->sitequota_model->getTicketUpdateList($ticketDetails->incident_number);
